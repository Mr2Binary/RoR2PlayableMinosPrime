...
namespace MinosMod.Modules {
    internal static class Language
    {
        ...
        private static void Language_collectLanguageRootFolders(List<string> obj) {
            string path = Path.Combine(Path.GetDirectoryName(MinosPlugin.instance.Info.Location), "Language");
        ...
        public static void PrintOutput(string fileName = "") {
            ...
            if (!string.IsNullOrEmpty(fileName)) {
                string path = Path.Combine(Directory.GetParent(MinosPlugin.instance.Info.Location).FullName, "Language", "en", fileName);
